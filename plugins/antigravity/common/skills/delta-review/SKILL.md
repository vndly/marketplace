...
Give each the complete change set, the shared baseline, its own brief, the reporting rules from steps 6 and 7, and **the static-reading constraint from the top of this skill** — reading, `git`, and search commands only, and no builds, tests, linters, formatters, or package managers. Collect every result before synthesis. If subagents are unavailable or not permitted, apply the same briefs sequentially yourself and disclose that fallback in the provenance line. A reviewer that is not told the static-reading constraint may run the suite to check its own finding. A and B overlap slightly by design; deduplicate at reporting time.
...
Collect every **Critical** or **Warning** finding — from the review subagents, or from your own pass if you took the inline tier — and deduplicate them. Use Antigravity subagents to assign **one skeptic per finding** — up to ten — running as many `invoke_subagent` tasks concurrently as capacity permits. Past ten, cluster the findings by file into at most ten groups and give one skeptic each group, asking for a separate verdict on every finding in it; say that you batched and how. Give each the finding, the change set, and **the brief the finding came from** — you spawned the subagent that produced it, so you know which; for an inline-tier finding it is whichever brief you were applying when you reached it, the shared baseline in step 3 or the lens brief whose rule it cites. A batched skeptic gets every brief its group draws on. If subagents are unavailable or not permitted, perform a distinct self-refutation pass for each finding and disclose that fallback in the provenance line. A lens finding cites a project rule that appears nowhere in the diff, so a skeptic who cannot see that rule cannot judge it. Give each skeptic this instruction:
