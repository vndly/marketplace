...
Use Antigravity subagents in parallel when they are available and permitted. Spawn bounded, independent tasks via `invoke_subagent` (delegating to subagents such as `research` or `self`), collect their results before synthesis, and use `send_message` only to clarify an existing assignment. Subagents may inspect files, trace flows, and propose or refute candidates, but they may only read files and run read-only search and `git` commands: they must not write repository files or run project code, tests, REPLs, or package managers. Execution-based verification is done by the coordinating agent under the Operating Rules and the worktree comparison in Phase 2. Include the Operating Rules verbatim in every subagent prompt. Discovery subagents may over-report candidates, but only the coordinating agent writes `BUG_FINDINGS.md`.
...
3. Select the audit scope. If the invocation already specifies a scope, use it without asking, first applying the `commit: <sha>` and `base: <ref>` validation below when the invocation supplies one. If the invocation names more than one scope, or conflicting `base:` and `commit:` values, ask which one to use; with no interactive response, use the default scope below and record the conflict. Text that only approves replacing `BUG_FINDINGS.md` is not a scope. Otherwise, use `ask_question` when it is available; if it is unavailable, ask the same questions directly in plain text and wait for the answer:
   - When a base ref was detected, call `ask_question` with the options `(Recommended) Diff vs <detected base>`, `Local changes`, and `Whole codebase`. The tool provides a free-text write-in option by default for a different base ref, a starting commit, or named paths, requiring `base: <ref>`, `commit: <sha>`, or `paths: <paths/globs>` so the intent is unambiguous; clarify any unlabeled answer before proceeding. Choosing the first option needs no base follow-up.
   - When no base ref was detected, state that in the question and call `ask_question` with the options `(Recommended) Whole codebase`, `Local changes`, and `Diff vs a branch`. A starting commit can be supplied through the tool's free-text write-in option as `commit: <sha>`. If `Diff vs a branch`, ask a follow-up question listing up to three existing local or remote branches, other than the current branch and its own remote-tracking refs, whose merge-base with `HEAD` is not `HEAD`, accepting a user-supplied ref through the tool's free-text write-in option.
...
5. Read the Antigravity instruction chain applicable to every included path: `GEMINI.md`, `AGENTS.md`, and rules under `.agents/rules/*.md` in each directory from the repository root toward that path. Then read project documentation, architecture material, schemas, and key contracts as evidence of intended behavior.
