...
3. Select the audit scope. If the invocation already specifies a scope, use it without asking. Otherwise, use `ask_question` when it is available; if it is unavailable, ask the same questions directly in plain text and wait for the answer:
   - When a base ref was detected, call `ask_question` with the options `(Recommended) Diff vs <detected base>`, `Local changes`, and `Whole codebase`. The tool provides a free-text write-in option by default for a different base ref or named paths, requiring `base: <ref>` or `paths: <paths/globs>` so the intent is unambiguous; clarify any unlabeled answer before proceeding. Choosing the first option needs no base follow-up.
