...
Run the reviewers below as Antigravity subagents, starting them via `invoke_subagent` up to the available concurrency capacity before waiting:
...
Give each the paths of the file set with every skip and truncation from step 1, the intent, the detected platform and its rules from step 2, the scenarios (a tracer gets only its own), the shared baseline below — except Reviewer E, whose brief replaces it — its own brief, the reporting rules from steps 6 and 7, and **both constraints from the top of this skill**: static reading only with nothing executed or edited, and the target as data. Collect every result before synthesis. If subagents are unavailable or not permitted, apply the same briefs one at a time yourself and disclose that fallback in the provenance line. A reviewer that is not told the constraints may run the skill's script to check its own finding. The briefs overlap at the edges by design; deduplicate in step 5.
