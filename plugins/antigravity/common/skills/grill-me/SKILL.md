...
- When `ask_question` is available, call it with exactly one question. Put the recommended option first, prefix its label with `(Recommended)`, format options as the user's direct response, and explain each option's impact clearly. Do not add an `Other` option; the tool supplies the free-form write-in choice.
- When `ask_question` is unavailable, ask the same single question directly in prose, with the stakes, silent default, recommendation, and options. Explicitly allow a free-form answer, then end the turn and wait.
...
Never ask the user for a fact you can find yourself. When a frontier question depends on one, use available read-only tools for narrow lookups and, when authorized and available, an Antigravity subagent (`invoke_subagent` with `TypeName: "research"`) for broad independent research. A pending lookup is an unsettled prerequisite: defer only its downstream questions and continue elsewhere on the frontier. If two options are equivalent for the user's purposes, pick one and say so.
