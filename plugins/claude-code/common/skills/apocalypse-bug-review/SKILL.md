...
3. Select the audit scope. If the invocation already specifies a scope, use it without asking, first applying the `commit: <sha>` validation below when the invocation supplies one. Otherwise, ask the user with `AskUserQuestion`:
   ...
   - When no base ref was detected, state that in the question and ask a primary question with the options `Local changes`, `Whole codebase`, `Diff vs a branch`, and `Named path(s)`. A starting commit can be supplied via `Other` as `commit: <sha>`. If `Diff vs a branch`, ask a follow-up question with the options `develop`, `main`, and a user-supplied ref via `Other`.
