...
3. Select the audit scope. If the invocation already specifies a scope, use it without asking. Otherwise, use `request_user_input` when it is available; if it is unavailable, ask the same questions directly in plain text and wait for the answer:
   - When a base ref was detected, ask a primary question with the options `Diff vs <detected base> (Recommended)`, `Local changes`, and `Whole codebase`. Use the tool-provided free-text `Other` for a different base ref or named paths, requiring `base: <ref>` or `paths: <paths/globs>` so the intent is unambiguous; clarify any unlabeled answer before proceeding. Choosing the first option needs no base follow-up.
   - When no base ref was detected, state that in the question and ask a primary question with the options `Whole codebase (Recommended)`, `Local changes`, and `Diff vs a branch`. Use the tool-provided free-text `Other` for named paths. If `Diff vs a branch`, ask a follow-up question with the options `develop` and `main`, accepting a user-supplied ref through `Other`.
