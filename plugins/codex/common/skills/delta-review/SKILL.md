...
Give each the complete change set, the shared baseline, its own brief, the reporting rules from steps 6 and 7, and **the static-reading constraint from the top of this skill** — reading, `git`, and search commands only, and no builds, tests, linters, formatters, or package managers. When briefs outnumber available slots, collect completed results and reuse idle agents with `followup_task`; use `send_message` only to clarify a running assignment. Collect every result before synthesis. If collaboration agents are unavailable or not permitted, apply the same briefs sequentially yourself and disclose that fallback in the provenance line. A reviewer that is not told the static-reading constraint may run the suite to check its own finding. A and B overlap slightly by design; deduplicate at reporting time.
